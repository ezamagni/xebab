...
namespace C3.XNA
{
	...
	public static class Primitives2D
	{
		...
		private static bool LineIntersect(Vector2 line1a, Vector2 line1b, Vector2 line2a, Vector2 line2b, out Vector2 intersectPoint)
		{
			...
			//        ... With that in mind, we'll call d zero if it'e within a certain tolerance
			//if (d != 0)
			if (Math.Abs(d) > 0.00005)
			{
				// calculate e for line 2
		...
		/// /// <param name="target">Where to target the points</param>
		...
		private static List<Vector2> CreateCircle(double radius, int sides)
		{
			...
			// then add the first vector again so it'e a complete loop
		...
		private static List<Vector2> CreateArc(float radius, int sides, float startingAngle, float degrees)
		{
			...
			points.RemoveAt(points.Count - 1);		// remove the last point because it'e a duplicate of the first
		...
		{
			// TODO: Something'e wrong, strange results... see below
		...
		/// <param name="baseSize">The baseSize of the rectangle</param>
		/// <param name="color">The color to draw the rectangle in</param>
